...
resource "aws_s3_bucket" "thing-deployment-bucket" {
  bucket = "${var.organization_name}-thing-deployment-${replace(var.thing_group, "_", "-")}-${data.aws_region.current.name}"
  # acl    = "private"
  # versioning {
  #   enabled = true
  # }
  tags = {
    Name = "${var.organization_name}-thing-deployment-${replace(var.thing_group, "_", "-")}-${data.aws_region.current.name}"
  }
}
